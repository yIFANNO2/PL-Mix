...
def load_image_label_list_from_npy(img_name_list):
    ...
    cls_labels_dict = np.load('/mnt/petrelfs/jinglinglin/Yifan/CAMMIX/voc12/cls_labels_collin.npy', allow_pickle=True).item()

    return [cls_labels_dict[int(img_name)] for img_name in img_name_list]
...
def load_img_name_list(dataset_path):
    ...
    img_name_list = [img_gt_name.replace('_', '') for img_gt_name in img_gt_name_list]
    img_name_list2 = [img_gt_name.split(' ')[0][-15:-4] for img_gt_name in img_gt_name_list]
...
class VOC12ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        name = self.img_name_list[idx]
        # name = name.split('')[0:3]+'_'+name.split('')[4:-1]
        #合并字符串
        name = name[0:4]+'_'+name[4:]
        # ''.join(name)
