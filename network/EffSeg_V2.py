...
class EfficientSeg(nn.Module):
    def __init__(self, num_classes, pretrained='efficientnet-b1', layers=1, MemoryEfficient=True, bifpn_channels=256, last_pooling=True, mode='enc'):
        ...
        # self.LSF=PSPModule
        # self.MotionLayer=PSPModule
        # self.MoladutionLayer=PSPModule
    ...
    def PCM(self, cam, f):
        ...
        # aff = F.relu(torch.matmul(f.transpose(1,2), f),inplace=False)
    ...
    def forward(self, x, cam='fm'):
        ...
        if cam == 'logits':
            ...
            # with torch.no_grad():
            #     f1 = F.interpolate(x, size=p7.shape[2:], mode='bilinear', align_corners=True)
            #     f2 = torch.relu(p5)
            #     f3 = torch.relu(p6)
            #     fs = torch.cat([f1, f2, f3], dim=1)
