...
if __name__ == '__main__':
    ...
    parser.add_argument("--weights", default='Your trained weights', type=str)
    parser.add_argument("--infer_list", default="data/VOCdevkit/VOC2012/ImageSets/Segmentation/train.txt", type=str)
    ...
    parser.add_argument("--out_npy", default='Path where npy is saved', type=str)
    ...
    if '.ckpt' in args.weights:
        ...
        model.load_state_dict(torch.load(args.weights, map_location=devise)['model_state_dict'], strict=False)
    else:
    ...
    infer_dataset = VOC12ClsDatasetMSF(args.infer_list, voc12_root=args.voc12_root,
                                                  scales=[0.45, 0.6, 0.8, 1, 1.2, 1.6, 1.8,2.0],
                                                  # scales=[0.45, 0.8, 1, 1.6, 1.8,2.0],
                                                  inter_transform=torchvision.transforms.Compose(
                                                       [np.asarray,
                                                        imutils.color_norm,
                                                        imutils.HWC_to_CHW]))
    #0.6, 0.8,1, 1.25, 1.6, 1.8, 2.   69.2
    #0.45, 0.6, 0.8, 1, 1.2, 1.6, 1.8,2.0 69.37
